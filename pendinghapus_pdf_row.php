...
$stmt = $koneksi->prepare("
    ...
    LEFT JOIN warga w 
        ON dp.nik COLLATE utf8mb4_general_ci = w.NIK COLLATE utf8mb4_general_ci
    LEFT JOIN keluarga k 
        ON (
            dp.no_kk COLLATE utf8mb4_general_ci = k.No_kk COLLATE utf8mb4_general_ci
            OR w.No_kk COLLATE utf8mb4_general_ci = k.No_kk COLLATE utf8mb4_general_ci
        )
    WHERE dp.id_pending = ?
      AND dp.aksi = 'hapus'
");
...
if (!$p) {
    die('Data pending hapus tidak ditemukan');
}

/* ================= DATA ================= */
$identitas = $p['tipe_data'] === 'warga' ? $p['nik'] : $p['no_kk'];
...
/* ================= CATATAN DIPERKECIL ================= */
$catatanRaw = $p['catatan'] ?: '-';
$catatan = strlen($catatanRaw) > 120
    ? substr($catatanRaw, 0, 120) . '...'
    : $catatanRaw;
...
.modal-header-sim {
    background:#dc3545;color:#fff;padding:14px;
...
<div class="modal-header-sim">DETAIL DATA PENGAJUAN HAPUS</div>

<div class="card-sim">
    <div class="row">
        <div class="col-6"><label>'.($p['tipe_data']==='warga'?'NIK':'No KK').'</label><div class="field">'.$identitas.'</div></div>
        <div class="col-6"><label>Nama</label><div class="field">'.$namaTampil.'</div></div>
    </div>

    <div class="row">
        <div class="col-6"><label>Alamat</label><div class="field">'.$alamat.'</div></div>
        <div class="col-6"><label>RT / RW</label><div class="field">'.$rt.' / '.$rw.'</div></div>
    </div>

    <div class="row">
        <div class="col-6"><label>Kelurahan</label><div class="field">'.$kelurahan.'</div></div>
        <div class="col-6"><label>Kecamatan</label><div class="field">'.$kecamatan.'</div></div>
    </div>

    <div class="row">
        <div class="col-8"><label>Catatan</label><div class="field">'.$catatan.'</div></div>
    </div>

    <div class="center">
        <label>Dokumen Pendukung</label><br>'.$dokumenHtml.'
    </div>
</div>
...
$dompdf->render();
$dompdf->stream("Pending_Hapus_{$id_pending}.pdf", ["Attachment" => false]);
exit;
